...
"""
The interface to load raw_log_data datasets. The datasets currently supported include
HDFS and BGL.
...
def load_HDFS(log_file,
              ...
              save_csv=False,
              window_size=0):
    """ Load HDFS structured raw_log_data into train and test data

    Arguments
    ---------
        log_file: str, the file path of structured raw_log_data.
    ...
    num_train_pos, num_test_pos = sum(y_train), sum(y_test)
...
def bgl_preprocess_data(para, raw_data, event_mapping_data):
    ...
    raw_log_data: list of (label, time)
    event_mapping_data: a list of event index, where each row index indicates a corresponding raw_log_data

    Returns:
    --------
    event_count_matrix: event count matrix, where each row is an instance (raw_log_data sequence vector)
    ...
    # get all the raw_log_data indexes in each time window by ranging from start_index to end_index
    ...
    print('There are %d raw_log_data events' % event_num)
